...
static const iocshArg testArg1 = { "getLinkRecordName", iocshArgString };
...
static const iocshFuncDef getLinkFuncDef = {
    "getLinkCreateRecord", 4, testArgs};
static void getLinkCallFunc(const iocshArgBuf *args)
{
    string provider("pva");
    string getLinkRecord("getLink");
    string linkedRecordName("doubleArray");
    char *sval = args[0].sval;
    if(sval) provider = string(sval);
    string providerNames(provider);
    sval = args[1].sval;
    if(sval) getLinkRecord = string(sval);
    sval = args[2].sval;
    ...
    GetLinkRecordPtr record = GetLinkRecord::create(
       pva,getLinkRecord,provider,linkedRecordName);
    if(record) 
        result = master->addRecord(record);
    if(!result) cout << "recordname" << " not added" << endl;
}

static void getLinkRegister(void)
{
    static int firstTime = 1;
    if (firstTime) {
        firstTime = 0;
        iocshRegister(&getLinkFuncDef, getLinkCallFunc);
    }
}

extern "C" {
    epicsExportRegistrar(getLinkRegister);
} 
