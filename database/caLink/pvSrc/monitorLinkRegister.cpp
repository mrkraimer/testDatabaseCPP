...
static const iocshArg testArg1 = { "monitorLinkRecordName", iocshArgString };
...
static const iocshFuncDef monitorLinkFuncDef = {
    "monitorLinkCreateRecord", 4, testArgs};
static void monitorLinkCallFunc(const iocshArgBuf *args)
{
    string provider("pva");
    string monitorLinkRecord("monitorLink");
    string linkedRecordName("doubleArray");
    char *sval = args[0].sval;
    if(sval) provider = string(sval);
    string providerNames(provider);
    sval = args[1].sval;
    if(sval) monitorLinkRecord = string(sval);
    sval = args[2].sval;
    ...
    MonitorLinkRecordPtr record = MonitorLinkRecord::create(
       pva,monitorLinkRecord,provider,linkedRecordName);
    if(record) 
        result = master->addRecord(record);
    if(!result) cout << "recordname" << " not added" << endl;
}

static void monitorLinkRegister(void)
{
    static int firstTime = 1;
    if (firstTime) {
        firstTime = 0;
        iocshRegister(&monitorLinkFuncDef, monitorLinkCallFunc);
    }
}

extern "C" {
    epicsExportRegistrar(monitorLinkRegister);
} 
