...
epicsExportAddress(rset,uint64RSET);

static void checkAlarms(uint64Record *prec);

static long init_record(struct dbCommon *pcommon, int pass)
{
    struct uint64Record *prec = (struct uint64Record *)pcommon;
...
#define indexof(field) uint64Record##field

static long get_units(DBADDR *paddr,char *units)
{
    uint64Record *prec=(uint64Record *)paddr->precord;
